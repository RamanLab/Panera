...
%% Step-3: Using PGMM in microbial community modelling (Hybrid modelling)
...
for i = 2:length(modelNames)
   ...
   % Check if the model is pan-genus model or species model
    if startsWith(current_file, 'pan')
        copyfile(fullfile(panPath, current_file), modelsPath);
    else
        copyfile(fullfile(agoraPath, current_file), modelsPath);
    end
end
...
numWorkers = 12;
[init, netSecretionFluxes, netUptakeFluxes, Y] = initMgPipe(modelsPath, normAbunFilePath, computeProfiles, 'resPath', resPath, 'dietFilePath', dietFilePath, 'saveConstrModels', saveConstrModels, 'numWorkers', numWorkers);
...
% resPath = analysisPath;
% normAbunFilePath = [folderPath 'tutorial/data/normalizedAbundances_sp.txt'];
